...
- **① 종합 대시보드** — 작업자별 `수량`·`토트 수` 그룹 막대그래프(이중 축) + 요약 타일
  (총수량 / 총토트 / 작업자수 / 토트당 평균 수량).
- **② 집품 수량 순위** · **③ 토트 수 순위** · **④ 종합 순위** — 3개의 가로 막대 순위 카드.
...
- **작업자 매핑** 버튼 → 표: 1열 = 집품작업자(코드), 2열 = 이름.
- 왼쪽 열은 수집된 코드로 자동 채워지며, 이름을 입력하고 **저장**하면 모든 차트 라벨이 이름으로 표시됩니다.
