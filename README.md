...
- **종합 대시보드** — 작업자별 `수량`(인디고)·`토트 수`(앰버) 그룹 막대그래프(이중 축) + 요약 타일
...
- **실시간 자동 새로고침** — 결과 화면 상단 토글로 켜고 끕니다(기본 켜짐·1분, 30초/1분/5분 선택). 켜지면
  주기마다 목록을 다시 조회해 **새로 생긴 토트만** 추가로 받아(상세는 링크별 캐시) 타일·순위·차트를 제자리 갱신하고
  "마지막 업데이트" 시각을 표시합니다. 설정은 브라우저에 저장됩니다.
- 모달은 화면을 거의 꽉 채우는 큰 크기이며, 헤더 ✕ 로 닫습니다.
